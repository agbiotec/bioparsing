...
my ( $unirefID, $unirefHEAD, $unirefCluster, $unirefTaxon, $isoform, @isoformData, $newHeader, $misc, $sequence, $crossIDs, $taxonID, $key, $value, $uniref_entry_flag, $cross_refs_flag, $kingdom_flag, $clusters_flag,@cluster_members,@taxon_members, $members_taxons, $cluster_ids, $kingdom, $db);
...
$clusters_flag = 0;
    ...
    while (<STDIN>) {
	    ...
	    $uniref_entry_flag = 0;
            ...
            if ($isoform eq "") {
	        #$newHeader = ">$db|$unirefID^|^$crossIDs^|^$cluster_ids^|^$members_taxons\@\@\@$kingdom $unirefCluster taxon:$taxonID {$unirefTaxon} $misc ";
	        $newHeader = ">$cluster_ids\@\@\@$kingdom";
	       }

            else {
	        #$newHeader = ">$db|$unirefID^|^$crossIDs^|^$cluster_ids^|^$members_taxons^|^$isoform\@\@\@$kingdom $unirefCluster taxon:$taxonID {$unirefTaxon} $misc ";
	        $newHeader = ">$cluster_ids\@\@\@$kingdom";
	       }
	...
	($key,$value) = split(/\t/, $_);
        ...
        # line is a uniref id mapping header
		 ...
		 #$members_taxons = "Cluster_Taxons|$members_taxons";
                 ...
                 @taxon_members = split(/,/,$members_taxons);
                 @cluster_members = split(/,/,$cluster_ids);
		 $cluster_ids = '';
                 foreach(@cluster_members) {
			if (length($_)>0) { 
		            #$cluster_ids = $cluster_ids.$db.'|'.$_.'^|';		
		            #$cluster_ids = $cluster_ids.$db.'|'.$_.'  taxon:'.$taxon_members[$i].' { } () ^|^';
		            $cluster_ids = $cluster_ids.$db.'|'.$_.'|'.$taxon_members[$i].'^|^';		
                        }
			$i++;
		 }
